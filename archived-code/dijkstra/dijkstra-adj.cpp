...
struct Graph {
    ...
    Graph(){}
    Graph(int size_i){
        vertex_vector.assign(size_i,list<Edge>() );
    }
    inline void InsertEnode(int v, Edge e){
...
void dijkstra(int source){
    ...
    while(heap.size()){
        ...
        for (list<Edge>::iterator i=g.vertex_vector[extend.num].begin();
            i!=g.vertex_vector[extend.num].end(); ++i){
            insert( (*i).to,extend.dis+(*i).val );
        }
    }
}
