...
/*  Output: The index of the target number in the number list and the number of         */
/*  comparisons using both algorithms.                                                  */
/*--------------------------------------------------------------------------------------*/
...
public class BinarySearch {
	...
	//define the index for numbers not found in the list to be -1
	static private int nBin,nLin;
	//nBin is the number of comparison using Binary Search. nLin is that using Linear Search.
	...
	static private int binary(int lb,int rb,String target){
		...
		if(numberList.get(i).equals(target)){
			...
			//if the target is smaller than middle element, it must be in the left half. 
			//Use recursion to continue searching.
		}
		else{
			return binary(i+1,rb,target);
			//if the target is larger than middle element, it must be in the right half. 
			//Use recursion to continue searching.
		}
	}
	...
	static private void nameListInput() throws IOException{
		//this method load the number list from the file.
		BufferedReader fbr=new BufferedReader(new FileReader("phone.txt"));
		//use BufferedReader+FileReader to input number list.
		numberList=new ArrayList<String>();
		while(fbr.ready()){
			numberList.add(fbr.readLine());
			//input all numbers in the file and store them in numberList.
		}
	}
	static private void processSearchRequest(BufferedReader br) throws IOException{
		//this method deal with user-interface and process the requests from the user.
		...
		if(index==FILE_NOT_FOUND){
			...
			//if the target is successfully found, tell the user the index.
		}
	...
	public static void main(String[] args) throws IOException{//main method
		nameListInput();//load number list from file.
		BufferedReader br=new BufferedReader(new InputStreamReader(System.in));
		//after loading all numbers, start interacting with user.
		while(true){
			processSearchRequest(br);//deal with user's request.
		}
	}
}
